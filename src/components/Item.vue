<template>
  <div>
    ...
    <div v-for="item in itemss" :key="item.question">
      <h2>{{ item.title }}</h2>
      ...
      <div class="buttondiv bt">
        ...
        <button
          ...
          @click="selected(index+1,$event)"
          v-for="(i ,index) in item.select"
          :key="item.answer"
        >{{ i }}</button>
      </div>

      <img
        src="../assets/consider.gif"
        class="img-responsive center-block"
        v-if="imgStatus=='consider'"
      >
      ...
      <div class="buttondiv">
        <button
          type="button"
          class="btn btn-primary btn-lg fr"
          v-if="current<44 && showNext"
          @click="nextPage"
        >Next test >></button>
      </div>
